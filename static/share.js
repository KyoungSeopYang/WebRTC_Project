...
var conference = function (config) {
    ...
    function defaultSocketResponse(response) {
        ...
        if (isGetNewRoom && response.roomToken && response.broadcaster) config.onRoomFound2(response);

        if (response.newParticipant) onNewParticipant2(response.newParticipant);
    ...
    function openSubSocket(_config) {
        ...
        var peerConfig = {
            oniceconnectionstatechange2: function (p) {
                if (!isofferer) return;

                if (p && p.iceConnectionState) {
                    config.oniceconnectionstatechange2(p.iceConnectionState);
                }
            },
            ...
            onRemoteStream2: function (stream) {
        ...
        function socketResponse(response) {
            ...
            if (response.left) {
                participants--;
                if (isofferer && config.onNewParticipant2) config.onNewParticipant2(participants);
        ...
        function selfInvoker() {
            ...
            if (isofferer && inner.sdp.type == 'answer') {
                ...
                participants++;
                if (config.onNewParticipant2) config.onNewParticipant2(participants);
    ...
    function onNewParticipant2(channel) {
...
function RTCPeerConnectionHandler(options) {
    ...
    if ('addStream' in peer) {
        peer.onaddstream = function (event) {
            ...
            // onRemoteStream2Ended(MediaStream)
            addStreamStopListener(remoteMediaStream, function () {
                if (options.onRemoteStreamEnded) options.onRemoteStreamEnded(remoteMediaStream);
            });

            // onRemoteStream2(MediaStream)
            if (options.onRemoteStream2) options.onRemoteStream2(remoteMediaStream);
    ...
    peer.oniceconnectionstatechange2 = peer.onsignalingstatechange = function () {
        if (peer && peer.isConnected && peer.iceConnectionState == 'failed') return;
        options.oniceconnectionstatechange2(peer);
    };
